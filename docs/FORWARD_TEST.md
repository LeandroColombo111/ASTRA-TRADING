...
- 2026-09-19: entre 18:52 y 19:04 UTC el bot fallo varias veces con `OperationalError` (base de datos) y systemd lo reinicio solo (4 reinicios), mientras se instalaba Hermes en la VM (disco al 84%). Sin posicion abierta y con las entradas bloqueadas en esos ciclos: sin efecto en la medicion. Disco ampliado de 10 GB a 30 GB (maximo del free tier, `pd-standard`) y sistema de archivos agrandado en linea; uso ahora 28%. Sin cambios de estrategia.
- 2026-09-21: **primera senal real** (09:00 UTC, largo BTC). La orden post-only fue cancelada por OKX en 1 ms (`cancelSource 31`: "would take liquidity") y la entrada se perdio; BTC subio ~1.8% despues. Cuenta como 1 senal perdida en la medicion. Cambio de EJECUCION (no de estrategia: mismas senales, mismos stops/objetivos, mismo tamano por riesgo): escalon de entrada post-only -> si OKX lo rechaza, reintento post-only con cotizacion nueva -> si vuelve a rechazar, entrada a mercado, siempre que el precio no se haya alejado mas de 0.3% del precio de la senal. Solo una *rechazo* dispara el escalon; una orden que queda descansando sin llenarse se cancela como antes y no se persigue. Este cambio hace que las entradas se parezcan mas al backtest (entrada a mercado). `configs/selected.json` sin cambios.
- 2026-09-25: cambio de ejecucion (no de estrategia): la vela cerrada se marca como consumida despues de todas las lecturas de OKX (posiciones, equity, ordenes, algos, precio) y antes de la primera escritura, en vez de al principio del ciclo; un error de lectura transitorio ya no descarta la senal de esa vela y sigue sin poder entrarse dos veces sobre la misma vela. Los ciclos fallidos ahora registran el mensaje de error. Mismas senales, stops, objetivos y tamano.
