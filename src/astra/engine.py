...
import numpy as np
import pandas as pd
from .directional_strategy import features
...
class Account:
    ...
    entry_effective_stop: float = 0.
    entry_effective_risk_fraction: float = 0.
...
def step(s, timestamp, bar, previous, current, p, risk, force_close=False):
    ...
    def close(price, reason):
        ...
        events.append({"entry_time":s.entry_time,"exit_time":str(timestamp),"side":int(side),"quantity":abs(s.qty),"entry":s.entry,"exit":fill,"net_pnl":pnl,"reason":reason,"entry_effective_stop":s.entry_effective_stop,"entry_effective_risk_fraction":s.entry_effective_risk_fraction})
        s.trades += 1
        s.qty = 0.
        s.trade_cost = 0.
        ...
        if direction and np.isfinite(atr) and atr > 0:
            ...
            if distance < entry:
                ...
                if quantity:
                    ...
                    post_entry_cash = s.cash-quantity*entry*fee
                    entry_drawdown_stop = entry+(s.peak*(1-risk.max_drawdown)-post_entry_cash)/s.qty
                    s.entry_effective_stop = max(s.stop,entry_drawdown_stop) if direction==1 else min(s.stop,entry_drawdown_stop)
                    s.entry_effective_risk_fraction = quantity*(abs(entry-s.entry_effective_stop)+entry*(2*fee+slip))/s.cash
    ...
    if s.qty:
        ...
        if stopped:  # When both touch, pessimistically stop first.
            ...
            if np.isfinite(atr) and s.peak_favorable_r >= p.trail_start_r:
