...
## 6. Aportes periódicos de capital — dos requisitos obligatorios

El capital no será fijo: se parte de ~1.000 USDT con aportes de ~300 USDT/mes. Eso rompe dos supuestos del código actual y **debe resolverse antes de operar con dinero real**.

### 6a. El corte por drawdown deja de funcionar con depósitos

En `service.py`, el drawdown se evalúa sobre el equity crudo del exchange:

```python
equity = self.x.equity()
peak = max(self.s.get('peak', equity), equity)
halted = self.s.get('halted', False) or equity <= peak*(1-self.r.max_drawdown)
```

Un depósito eleva `equity` y, por lo tanto, `peak`. Estando -20% sobre 3.000 USDT, un aporte de 300 USDT hace que el drawdown aparente caiga a ~-10%: **el kill switch no dispara cuando debería, y falla precisamente en los meses malos.**

Requisito: llevar un registro de flujos de capital (depósitos y retiros, con timestamp e importe) y calcular el drawdown sobre una **curva de equity ajustada por flujos**, no sobre el saldo. El `peak` se recalcula sobre esa curva ajustada. Un depósito nunca debe reducir el drawdown medido ni levantar un halt existente.

### 6b. La medición de performance debe ser time-weighted

Con aportes de ~30% mensual sobre el capital inicial, el saldo de la cuenta sube aunque la estrategia pierda. Juzgar el bot por el balance produce un falso positivo garantizado durante los primeros meses.

Requisito: todo reporte de performance usa **retorno time-weighted (TWR)**, encadenando los sub-períodos entre flujos de capital. Los depósitos se registran como eventos de capital, nunca como P&L. El reporte debe mostrar en líneas separadas: capital aportado acumulado, P&L de la estrategia, y saldo total. **Prohibido reportar el crecimiento del saldo como resultado de la estrategia.**

### 6c. Piso técnico y secuencia

Los perpetuos de OKX se negocian en contratos indivisibles (`ctVal`). Un contrato de BTC-USDT-SWAP es 0,01 BTC ≈ 785 USDT a precios actuales — el 78% de una cuenta de 1.000 USDT. **El modelo debe consultar la tabla real de `ctVal` y `minSz` de todos los instrumentos admitidos y verificar, antes de fondear, que el nocional objetivo por posición sea ≥ 1 contrato en cada símbolo del universo.** Cualquier símbolo cuyo contrato mínimo supere el 5% del capital queda excluido del universo operable para ese nivel de capital.

Consecuencia práctica: con ~30 posiciones simultáneas, el piso ejecutable de este diseño está en **~3.000 USDT**. Hasta llegar ahí, el sistema corre exclusivamente en demo. La validación prospectiva mínima (3 meses) y la acumulación de capital hasta el piso (~7 meses) corren en paralelo; no se adelanta el fondeo.

### 6d. Órdenes maker

Las comisiones reales de la cuenta, cacheadas en `data/okx/demo_fees.json`, son **maker 2 bps / taker 5 bps**. El v1 murió con los costos consumiendo el 94% del bruto operando siempre a mercado. A este tamaño de posición el libro no se mueve, así que la ejecución pasiva es viable de forma sistemática.

Requisito: el diseño de ejecución usa **órdenes límite post-only** como modo primario, con un fallback a mercado acotado por tiempo y una política explícita de fills perdidos. El backtest debe modelar los fills no ejecutados — no asumir que toda orden límite se llena, que es la forma más común de inflar un backtest de ejecución pasiva.

---

## 7. Nota sobre el trabajo del run abortado
